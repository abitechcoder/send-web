...
const SearchFiles = ({ files }: Props) => {
  ...
  const handleSearch = () => {
    ...
    if (searchText) {
      filteredItems = files.filter((item) => {
  ...
  return (
    <section className={`bg-white ${layout.section}`}>
      <section className="grid grid-cols-1 lg:grid-cols-[1fr_150px] gap-4">
        ...
        <div className="h-[50px]">
          <button
            ...
            className="w-full h-full bg-secondary text-white rounded-sm"
          >
            Search
          </button>
        </div>
      ...
      <section className="pt-8">
        <h2 className={`font-barlow ${styles.heading2Barlow} `}>
          Search Results:
        </h2>
        {filteredFiles.length === 0 ? (
          <div className="grid place-items-center py-8 bg-lightgrey mt-4 rounded-md">
